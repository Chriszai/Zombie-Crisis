...
import model.Bullet4;


public class PlayerView1 extends ImageView {
	...
	ArrayList<Bullet4> bullets = new ArrayList<>();
	...
	public PlayerView1(Pane pane, ArrayList<Zombies> zombies, ArrayList<ZombiesView1> zombiesView, int scores) {
	...
	public void handlePressed(KeyEvent keyEvent) {
		...
		if (keyEvent.getCode().equals(KeyCode.SLASH) && shootState == true) {
	...
	public void shoot(Player who) {
		Bullet4 bullet = new Bullet4(who, width, height, direction);
...
//      ||||||||||||||||||||||||||||||||||||||||||||||||||||||
		if (down) {
			if (direction == Direction.DOWN && PlayScene2.UP == false) {
			...
			if (direction == Direction.UP && PlayScene2.DOWN == false) {
			...
			if (direction == Direction.LEFT && PlayScene2.RIGHT == false) {
			...
			if (direction == Direction.RIGHT && PlayScene2.LEFT == false) {
